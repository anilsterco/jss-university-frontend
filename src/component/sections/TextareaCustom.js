...
function MobileTab({ item, activeTabs, setActiveTabs }) {
    const accordionRefs = useRef([]);

    const handleAccordion = (index) => {
        const isOpening = activeTabs[item.id] !== index;

        setActiveTabs((prev) => ({
            ...prev,
            [item.id]: isOpening ? index : null,
        }));
                ...
                {item?.side_menus?.map((menu, index) => (
                    ...
                    >
                        ...
                        >
                            ...
                            <svg
                                className={`accordion_arrow ${activeTabs[item.id] === index
                                        ? "rotate"
                                        : ""
                                    }`}
                                stroke="currentColor"
                        ...
                        <div
                            className={`accordion_content left_side_content ${activeTabs[item.id] === index
                                    ? "open"
                                    : ""
                                }`}
                        >
...
function DesktopTab({ item, activeIndex, activeTabs, setActiveTabs }) {
    return (
        <div className={''}>
            <div className="container">
                                ...
                                >
                                    {
                                        isMobile ?
                                            <>
                                                {
                                                    isMobile && item?.side_menus?.length > 1 ?
                                                        <MobileTab item={item} activeIndex={activeIndex} activeTabs={activeTabs} setActiveTabs={setActiveTabs} />
                                                        :
                                                        <DesktopTab item={item} activeIndex={activeIndex} activeTabs={activeTabs} setActiveTabs={setActiveTabs} />
                                                }
                                            </>
                                            :
                                            <DesktopTab item={item} activeIndex={activeIndex} activeTabs={activeTabs} setActiveTabs={setActiveTabs} />
